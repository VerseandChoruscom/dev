...
// DEBUGGING: Check for lyrics.json
$json_path = __DIR__ . '/lyrics.json';
$json_exists = file_exists($json_path);
$json_content = $json_exists ? file_get_contents($json_path) : '[]';

// Fallback: If empty or false, set to empty array string
if (!$json_content) {
    $json_content = '[]';
}
?>
<!-- DEBUG INFO:
JSON Path: <?php echo $json_path; ?>
File Exists: <?php echo $json_exists ? 'Yes' : 'No'; ?>
Content Length: <?php echo strlen($json_content); ?> characters
-->
<!DOCTYPE html>
<html lang="en">
<head>
...
<body class="min-h-screen flex flex-col">
    ...
    <script>
        ...
        // Instead of include, we inject the JSON content directly.
        const ALL_SONGS_DATA = <?php echo $json_content; ?>;
        ...
        function createStandardCardHtml(song) {
            ...
            return `
                <div id="song-${song.id}" data-song-id="${song.id}" class="song-card p-4 bg-gray-800 rounded-lg shadow-xl hover:shadow-2xl transition duration-300 scratched-border">
                    ...
                    <!-- SHORT DESCRIPTION PREVIEW: Uses short-description-preview CSS class for max 4 lines -->
